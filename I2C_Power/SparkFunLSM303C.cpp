...
{
  uint8_t value, mag_id;
  if( MAG_ReadReg(MAG_WHO_AM_I, value) )
  {
    return IMU_HW_ERROR;
  }
  mag_id = value;
  debug_print("Reading whoami id= 0x");
  debug_printlns(value, HEX);
  if( value != MAG_WHO_AM_I_ID_VALUE)
  {
    return IMU_HW_ERROR;
  }
...
//Duplicate of the above but this time for the accelerometer.
status_t  LSM303C::acc_whoami()
{
  uint8_t value;
  if( ACC_ReadReg(ACC_WHO_AM_I, value) )
  {
    return IMU_HW_ERROR;
  }
  debug_print("Reading whoami id= 0x");
  debug_printlns(value, HEX);
  if( value != ACC_WHO_AM_I_ID_VALUE)
  {
    return IMU_HW_ERROR;
  }
